...
dhis2.db.tmpl = {
    ...
    "<a href='javascript:dhis2.db.viewFullScreen( \"${itemId}\", \"reportTable\", \"${name}\" )'>${i18n_full}</a>" +
        ...
        if (undefined !== data.dashboards) {
            ...
            var dashboardList = [];
              for(let item of data.dashboards){
                 let values = item.displayName.split(" ")
                 dashboardList.push(values["0"])
              }
              let arr = [...new Set(dashboardList)]
              for (let dashboard of arr)
                { 
...
// dhis2.db. getOptionValue = function() {
...
dhis2.db.viewFullScreen = function (id, type, name) {
    let  listID = 'li-'+id
    var html = $("<div />").append($(`#${listID}`).clone()).html()
      let test = html.replace("item", "item test")
    $(".modal").show();
    $("#myModal").show();
    $("#contentID").html(test)
}
function closeModal(){
    // $(".table").css({ "width":"421px", "height":"342px"});
    $(".modal").hide();
    $("#myModal").hide();
}
